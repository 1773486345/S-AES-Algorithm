...
public class S_AES_3 {
    public static String W0,W0_,W0__;
    public static String W1,W1_,W1__;
    public static String W2,W2_,W2__;
    public static String W3,W3_,W3__;
    public static String W4,W4_,W4__;
    public static String W5,W5_,W5__;
    ...
    public static void get_Key(){
        System.out.println("请输入主密匙(48bit):");
        Scanner sc =new Scanner(System.in);
        String main_Key=sc.nextLine();
        String main_Key1= main_Key.substring(0,16);
        String main_key2=main_Key.substring(16,32);
        ...
        W0_= main_Key.substring(16,24);
        W1_= main_Key.substring(24,32);
        W2_=xor(W0_,g(W1_,1));
        W3_=xor(W1_,W2_);
        W4_=xor(W2_,g(W3_,2));
        W5_=xor(W4_,W3_);

        W0__= main_Key.substring(32,40);
        W1__= main_Key.substring(40,48);
        W2__=xor(W0__,g(W1__,1));
        W3__=xor(W1__,W2__);
        W4__=xor(W2__,g(W3__,2));
        W5__=xor(W4__,W3__);
    ...
    public static void encrypt(){
        ...
        Plainttext=replace(Plainttext,1);//半字节替代
        Plainttext=move_hang(Plainttext);//行移位
        Plainttext=xor(Plainttext,W4+W5);//轮秘钥加

        //第二重加密
        //第一轮
        Plainttext=xor(Plainttext,W0_+W1_);//轮秘钥加
        Plainttext=replace(Plainttext,1);//半字节替代
        Plainttext=move_hang(Plainttext);//行移位
        Plainttext=multiply_lie(Plainttext,1);//列混淆
        //第二轮
        Plainttext=xor(Plainttext,W2_+W3_);//轮秘钥加
        Plainttext=replace(Plainttext,1);//半字节替代
        Plainttext=move_hang(Plainttext);//行移位
        Plainttext=xor(Plainttext,W4_+W5_);//轮秘钥加

        //第3重加密
        //第一轮
        Plainttext=xor(Plainttext,W0__+W1__);//轮秘钥加
        Plainttext=replace(Plainttext,1);//半字节替代
        Plainttext=move_hang(Plainttext);//行移位
        Plainttext=multiply_lie(Plainttext,1);//列混淆
        //第二轮
        Plainttext=xor(Plainttext,W2__+W3__);//轮秘钥加
        Plainttext=replace(Plainttext,1);//半字节替代
        Plainttext=move_hang(Plainttext);//行移位
        String ciphertext=xor(Plainttext,W4__+W5__);//轮秘钥加
    ...
    public static void decrypt(){
        ...
        //第一重解密
        //第一轮
        ciphertext=xor(ciphertext,W4__+W5__);//轮秘钥加
        ciphertext=move_hang(ciphertext);//行移位
        ciphertext=replace(ciphertext,2);//半字节替代
        ciphertext=xor(ciphertext,W2__+W3__);//轮秘钥加
        ciphertext=multiply_lie(ciphertext,2);//列混淆
        //第二轮
        ciphertext=move_hang(ciphertext);//行移位
        ciphertext=replace(ciphertext,2);//半字节替代
        ciphertext=xor(ciphertext,W0__+W1__);//轮秘钥加

        //第2重解密
        //第一轮
        ciphertext=xor(ciphertext,W4_+W5_);//轮秘钥加
        ciphertext=move_hang(ciphertext);//行移位
        ciphertext=replace(ciphertext,2);//半字节替代
        ciphertext=xor(ciphertext,W2_+W3_);//轮秘钥加
        ciphertext=multiply_lie(ciphertext,2);//列混淆
        //第二轮
        ciphertext=move_hang(ciphertext);//行移位
        ciphertext=replace(ciphertext,2);//半字节替代
        ciphertext=xor(ciphertext,W0_+W1_);//轮秘钥加
